...
Local embedding provider running models via [Ollama](https://ollama.com). GPU-accelerated, private, and proven on multi-million LoC codebases.
...
:::warning macOS: use Ollama.app for GPU acceleration
On macOS, the `brew install ollama` CLI-only package **does not include GPU support**. Embeddings will run on CPU and be significantly slower.

To use Metal GPU acceleration, install the full [Ollama.app](https://ollama.com/download/mac):
...
For advanced GPU setup (Docker, remote Ollama, exposed GPU), see the [Local & Exposed GPU Setup](../local-exposed-gpu-setup) guide.
:::
