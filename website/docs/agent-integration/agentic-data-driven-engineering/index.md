...
| `blameDominantAuthorPct` (label) | `concentrated` / `silo` / `deep-silo` | (between) | `shared` |
...
Code generated in a vacuum passes tests but fails code review. The
`blameDominantAuthor` signal (live-line ownership from `git blame HEAD`) tells
the agent whose patterns to match — that author's lines are the ones still in
the file, so their style is what's actually there.
...
| Ownership profile (live-line, `blame*`) | Agent behavior |
|-------------------|----------------|
| `blameDominantAuthorPct` label = `silo` or `deep-silo`, `blameContributorCount` label = `solo` | **Strong silo** — match this author's style exactly. Don't refactor, don't introduce new patterns. The author will review this code. |
| `blameDominantAuthorPct` label = `concentrated`, `blameContributorCount` label = `pair` | **Clear owner** — follow the dominant author's patterns with minor flexibility. |
| `blameDominantAuthorPct` label = `shared`, `blameContributorCount` label = `team` or `crowd` | **Shared area** — generate neutral, conventional code. Follow project-wide conventions rather than any single author's style. Opportunity to introduce unifying patterns. |

:::tip When `blame*` and `recent*` disagree
If the live-line owner (`blameDominantAuthor`) is no longer the recent
committer (`recentDominantAuthor`), a knowledge handoff is in progress. Match
the live-line owner's style when generating code (their lines define what's
"local"), but route review to the recent committer (they're mentally loaded
in right now).
:::
...
| **Knowledge silo + churn** | `blameDominantAuthorPct` label = `silo` or `deep-silo` + `chunkCommitCount > 5` | Request review from the live-line owner before merging. Do not restructure without knowledge transfer. |
