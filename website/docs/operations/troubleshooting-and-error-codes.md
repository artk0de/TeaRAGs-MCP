...
| **A file skipped: chunker worker stopped answering** | `INGEST_WORKER_TIMEOUT` | A chunker worker took a file and did not answer within `CHUNKER_WORKER_TIMEOUT_MS` (default 120 s). Typical cause: a very large or minified file whose parse legitimately runs that long, or a tree-sitter native crash or deadlock. The hung worker is recycled and only that file is skipped — it is not quarantined, and the rest of the run continues. For a genuinely large file raise `CHUNKER_WORKER_TIMEOUT_MS` (`0` removes the bound); if an ordinary file hangs, report it together with the file. |
| **`--force-enrichments` stopped before rebuilding anything** | `INGEST_SEED_MARKER_UNREADABLE` | The recompute could not read the collection's indexing marker, so it cannot tell whether a worktree seed is still pending — and stamping over a pending seed would roll the new versions back on the next incremental. Typical cause: a transient Qdrant failure on that read — a 5xx, a timeout, or the embedded daemon still starting or recovering. Nothing was rebuilt or stamped: retry the same command once Qdrant answers. If it keeps failing, check that Qdrant is reachable and read its logs. |
...
| `INGEST_WORKER_TIMEOUT`                | `WorkerTimeoutError`         | 504  | `{pool}` worker did not respond for "`{request}`" within `{timeoutMs}`ms | The hung worker was recycled. Raise `CHUNKER_WORKER_TIMEOUT_MS` for a genuinely large file; otherwise report the crash/deadlock |
| `INGEST_SEED_MARKER_UNREADABLE`        | `WorktreeSeedMarkerUnreadableError` | 503 | Could not read the indexing marker of "`{collectionName}`" to check for a pending worktree seed | Nothing was stamped or rebuilt. Retry once Qdrant answers |
