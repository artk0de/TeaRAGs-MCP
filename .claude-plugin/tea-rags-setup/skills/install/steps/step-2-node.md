...
- `manual_required` (exit code 2) → check `currentVersion` in result. If not
  null, include in the message (e.g. "Node.js 22+ required, found v18.19.0").
  Use AskUserQuestion:
  ```
  question: "Node.js 22+ is required{. Found vX.Y.Z — upgrade needed | but not installed}. How would you like to install it?"
  options: [
    ...
    { label: "Already installed", description: "I've already installed Node.js 22+ — just re-detect" }
  ]
  ```
...
**If "Already installed"** → re-run detect-environment.sh, verify version >= 22.
