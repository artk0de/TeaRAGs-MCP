...
- **Python** — 9-strategy chain (super, clsMember, selfField, selfMember,
  ...
  callee's recorded return read when the project declares exactly one definition
  of the name) + `-> Self` recorded as a marker and substituted with the class
  the RECEIVER names + kernel return inference over return statements +
  subtype-gated naming-convention receiver typing + class-body attribute typing
  from declared-or-import-bound constructors + class fields addressed both
  ...
  candidates gated to same-language, bare-callable, non-builtin definitions
