...
search-cascade). Signals are scoped: `signals[language][signal][scope].labelMap`
where scope is `"source"` or `"test"`. Prefer the chunk's language + scope
thresholds when available. If absent in context, call `get_index_metrics`
yourself. Map raw signal values to the nearest labelMap threshold. See
