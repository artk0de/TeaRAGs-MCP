...
describe("flagTrackBPriorities", () => {
  it("flags a category whose mismatch rate clears the threshold on enough evidence", () => {
    const input = [...rows("missed", 30, { categories: ["unionNarrowing"] }), ...rows("match", 10, { categories: ["unionNarrowing"] })];
  ...
  it("withholds a flag from a well-covered category the chain already agrees with", () => {
    const input = [...rows("match", 99, { categories: ["generic"] }), ...rows("missed", 1, { categories: ["generic"] })];
  ...
  it("honours caller-supplied evidence and rate thresholds", () => {
    ...
    const flagged = flagTrackBPriorities(tallyBy(input, (r) => r.categories), { minOracle: 4, minMismatchRate: 0.5 });
...
describe("findUncoveredCategories", () => {
  it("names the expected categories the corpus produced no call site for", () => {
    ...
    expect(findUncoveredCategories(tallies, ["generic", "jsx", "structuralTyping"])).toEqual(["jsx", "structuralTyping"]);
  });
