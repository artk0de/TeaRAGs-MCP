...
import { assert, createTestFacades, createTestFile, resources, searchCode, section, sleep } from "../helpers.mjs";
...
`;
  ...
  const stats = await ingest.indexCodebase(TEST_DIR, { forceReindex: true });
  ...
  const userResults = await searchCode(explore, TEST_DIR, "UserService getUser");
  ...
  const productResults = await searchCode(explore, TEST_DIR, "ProductService price");
  assert(productResults.length > 0, `Search finds ProductService: ${productResults.length} results`);

  // Check index status
  const status = await ingest.getIndexStatus(TEST_DIR);
...
`;
  ...
  const addStats = await ingest.reindexChanges(TEST_DIR);
  assert(addStats.filesAdded >= 1, `New file detected: ${addStats.filesAdded} added`);

  const orderResults = await searchCode(explore, TEST_DIR, "OrderService createOrder");
...
`;
  ...
  const modifyStats = await ingest.reindexChanges(TEST_DIR);
  assert(modifyStats.filesModified >= 1, `Modified file detected: ${modifyStats.filesModified} modified`);

  const deleteResults = await searchCode(explore, TEST_DIR, "deleteUser");
  ...
  const deleteStats = await ingest.reindexChanges(TEST_DIR);
  ...
  const _deletedResults = await searchCode(explore, TEST_DIR, "ProductService getProduct");
