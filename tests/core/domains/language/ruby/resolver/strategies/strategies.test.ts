...
import { SUPER_RECEIVER_SENTINEL, ZEITWERK_PREFIX } from "../../../../../../../src/core/domains/language/ruby/walker/walker.js";
...
describe("RubyArRelationGuardSymbolResolutionStrategy", () => {
  ...
  it("DROPS when the receiver text is an AR::Relation chain — guard", () => {
    ...
    const outcome = strat.attempt(
      { callText: "Product.where(active: true).result", receiver: "Product.where(active: true)", member: "result", startLine: 1 },
      ctx({ symbolTable }),
    );
    expect(outcome.kind).toBe("drop");
  });
...
describe("RubyBareCallSymbolResolutionStrategy", () => {
  ...
  it("filters out non-ruby file candidates (cross-language pollution — bug pl7k)", () => {
    const symbolTable = tableWith(["vendor/assets/javascripts/d3.js", [sym("map", "map", "vendor/assets/javascripts/d3.js", [])]]);
  ...
  it("prefers the same-enclosing-class candidate when the short-name is ambiguous (bug t5iw)", () => {
    const symbolTable = tableWith(
      ["app/concerns/web_request_concern.rb", [sym("WebRequestConcern#user_agent", "user_agent", "app/concerns/web_request_concern.rb", ["WebRequestConcern"])]],
      ["app/agents/phantom_js_cloud_agent.rb", [sym("Agents::PhantomJsCloudAgent#user_agent", "user_agent", "app/agents/phantom_js_cloud_agent.rb", ["PhantomJsCloudAgent"])]],
    );
    const outcome = strat.attempt(
    ...
    expect(outcome).toEqual({
      kind: "resolved",
      target: { targetRelPath: "app/agents/phantom_js_cloud_agent.rb", targetSymbolId: "Agents::PhantomJsCloudAgent#user_agent" },
    });
  });
