...
describe("ChunkDensitySignal", () => {
  ...
  it("reads methodDensity from payload and normalizes", () => {
    ...
    const raw = { chunkType: "function", methodLines: 100, methodDensity: 60 };
    expect(signal.extract(raw)).toBeCloseTo(0.5, 5);
  });

  it("uses adaptive bound from ctx when provided", () => {
    const raw = { chunkType: "function", methodLines: 100, methodDensity: 50 };
  ...
  it("returns 0 when methodDensity is missing", () => {
    expect(signal.extract({ chunkType: "function", methodLines: 100 })).toBe(0);
  });

  it("returns 0 when methodDensity is 0", () => {
    expect(signal.extract({ chunkType: "function", methodLines: 100, methodDensity: 0 })).toBe(0);
  });

  it("returns 0 when methodLines is missing (non-method chunks like docs)", () => {
    expect(signal.extract({ chunkType: "function", methodDensity: 240 })).toBe(0);
  });

  it("returns 0 when methodLines is 0", () => {
    expect(signal.extract({ chunkType: "function", methodLines: 0, methodDensity: 60 })).toBe(0);
  });

  it("clamps to 1.0 when methodDensity exceeds bound", () => {
    // normalize(240, 120) = min(1, 2) = 1
    expect(signal.extract({ chunkType: "function", methodLines: 100, methodDensity: 240 })).toBeCloseTo(1.0, 5);
  });

  // --- Adaptive size threshold ---

  it("returns 0 when methodLines below floor (20 lines) without collectionStats", () => {
    expect(signal.extract({ chunkType: "function", methodLines: 15, methodDensity: 100 })).toBe(0);
    expect(signal.extract({ chunkType: "function", methodLines: 19, methodDensity: 100 })).toBe(0);
  });

  it("returns density when methodLines at floor (20 lines) without collectionStats", () => {
    expect(signal.extract({ chunkType: "function", methodLines: 20, methodDensity: 60 })).toBeGreaterThan(0);
  });
  ...
  it("returns 0 when methodLines below p50 from collectionStats", () => {
    ...
    expect(signal.extract({ chunkType: "function", methodLines: 50, methodDensity: 80 }, ctx)).toBe(0);
  });

  it("returns density when methodLines above p50", () => {
    // p50 = 100, methodLines=150 > 100 → compute density
    const ctx = ctxWithP50(100, 120);
    expect(signal.extract({ chunkType: "function", methodLines: 150, methodDensity: 60 }, ctx)).toBeGreaterThan(0);
  });

  it("uses floor when p50 is smaller", () => {
    // p50 = 10, threshold = max(20, 10) = 20 (floor wins)
    const ctx = ctxWithP50(10, 120);
    expect(signal.extract({ chunkType: "function", methodLines: 18, methodDensity: 80 }, ctx)).toBe(0);
    expect(signal.extract({ chunkType: "function", methodLines: 25, methodDensity: 60 }, ctx)).toBeGreaterThan(0);
  });

  it("always returns value in 0-1 range", () => {
    const values = [
      signal.extract({ chunkType: "function", methodLines: 100, methodDensity: 10 }),
      signal.extract({ chunkType: "function", methodLines: 100, methodDensity: 500 }),
      signal.extract({ chunkType: "function", methodLines: 100, methodDensity: 60 }),
    ];
  ...
  it("returns 0 for class chunks", () => {
    expect(signal.extract({ chunkType: "class", methodLines: 100, methodDensity: 80 })).toBe(0);
  });

  it("returns 0 for interface chunks", () => {
    expect(signal.extract({ chunkType: "interface", methodLines: 100, methodDensity: 80 })).toBe(0);
  });

  it("returns 0 for block chunks", () => {
    expect(signal.extract({ chunkType: "block", methodLines: 100, methodDensity: 80 })).toBe(0);
  });

  it("returns 0 when chunkType is missing", () => {
    expect(signal.extract({ methodLines: 100, methodDensity: 80 })).toBe(0);
  });
});
