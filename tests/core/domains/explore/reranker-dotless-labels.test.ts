 ...
 * a key carrying no dots (`moduleLines`, `moduleMethodCount`, `memberCount`,
...
describe("Reranker — overlay labels for dotless payload keys", () => {
  ...
  it("labels every dotless signal in the same overlay, not just the first", async () => {
    ...
    reranker.setCollectionStats(
      statsFor([
        ...
        ["moduleMethodCount", { 50: 3, 75: 15, 95: 30 }],
      ]),
    );

    const ranked = await reranker.rerank(
      [resultWith({ moduleLines: 120, moduleMethodCount: 69 })],
      "godModule",
      "rank_chunks",
    );
    ...
    expect(file?.moduleMethodCount).toEqual({ value: 69, label: "god-module" });
  });

  it("resolves the label against the threshold walk, not by saturating at the top", async () => {
    const reranker = new Reranker(staticDerivedSignals, presets, payloadSignals);
    reranker.setCollectionStats(statsFor([["moduleMethodCount", { 50: 3, 75: 15, 95: 30 }]]));

    const ranked = await reranker.rerank([resultWith({ moduleMethodCount: 20 })], "godModule", "rank_chunks");

    // 20 >= p75 (15) but < p95 (30) — "busy", the middle tier.
    expect(ranked[0].rankingOverlay?.file?.moduleMethodCount).toEqual({ value: 20, label: "busy" });
  });
  ...
  it("still leaves a dotless signal bare when the collection carries no stats for it", async () => {
    ...
    const ranked = await reranker.rerank(
      [resultWith({ moduleLines: 1589, moduleMethodCount: 69 })],
      "godModule",
      "rank_chunks",
    );

    // moduleLines has stats and gets a label; moduleMethodCount has none and must
    // stay a plain number rather than inventing a tier.
    expect(ranked[0].rankingOverlay?.file?.moduleLines).toEqual({ value: 1589, label: "god-module" });
    expect(ranked[0].rankingOverlay?.file?.moduleMethodCount).toBe(69);
  });
