...
describe("QuarantineStore", () => {
  ...
  describe("markFailed", () => {
    it("should persist a single file entry that load() reads back", async () => {
      await store.markFailed("src/foo/oversized.ts", new ChunkOversizedError("src/foo/oversized.ts", "chunk 12480 > 8192"));
    ...
    it("should leave no leftover .tmp file after an atomic write", async () => {
      ...
      const dirEntries = await fs.readdir(join(snapshotDir, collectionName));

      expect(dirEntries.some((name) => name.endsWith(".tmp"))).toBe(false);
      expect(dirEntries).toContain("quarantine.json");
    });
  });
