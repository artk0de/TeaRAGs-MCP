...
describe("computeCollectionStats distributions", () => {
  ...
  describe("dedupeByFile", () => {
    ...
    it("counts a many-chunk file once when the signal declares dedupeByFile", () => {
      ...
      const stats = result.perSignal.get("moduleMethodCount")!;
    ...
    it("keeps every aggregate file-weighted, not only the percentiles", () => {
      const stats = computeCollectionStats(skewedPoints(), fileScoped, ALL_ACCS).perSignal.get("moduleMethodCount")!;
    ...
    it("does not move any aggregate when one file's chunk count changes", () => {
      const thin = computeCollectionStats(skewedPoints(3), fileScoped, ALL_ACCS).perSignal.get("moduleMethodCount")!;
      const fat = computeCollectionStats(skewedPoints(400), fileScoped, ALL_ACCS).perSignal.get("moduleMethodCount")!;
    ...
    it("dedupes the per-language bucket as well as the global one", () => {
      ...
      expect(result.perSignal.get("moduleMethodCount")!.count).toBe(17);
