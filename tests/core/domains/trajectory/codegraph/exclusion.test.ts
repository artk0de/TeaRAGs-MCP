...
describe("buildCodegraphExclusionFilter", () => {
  it("always matches conventional test paths across languages", () => {
  ...
  // bd tea-rags-mcp-6xxh5 — test exclusion is an invariant, not a knob. A graph
  // over test files answers no architectural question (a test calls production
  // code and nothing calls it), so there is no configuration under which they
  // belong in it.
  it("excludes tests with an empty customPatterns list — the exclusion is unconditional", () => {
    const ig = buildCodegraphExclusionFilter({ customPatterns: [] });
    expect(ig.ignores("tests/foo.ts")).toBe(true);
    expect(ig.ignores("src/foo.test.ts")).toBe(true);
    expect(ig.ignores("pkg/test_user.py")).toBe(true);
    expect(ig.ignores("internal/repo_test.go")).toBe(true);
    // Arbitrary application files still pass — the filter is not a blanket ban.
    expect(ig.ignores("anything.ts")).toBe(false);
  });

  it("always excludes Rails-generated db/schema.rb regardless of options", () => {
    const igBare = buildCodegraphExclusionFilter({ customPatterns: [] });
    expect(igBare.ignores("db/schema.rb")).toBe(true);
    expect(igBare.ignores("backend/db/schema.rb")).toBe(true);
    // Unrelated custom patterns don't disturb the invariant.
    const igCustom = buildCodegraphExclusionFilter({ customPatterns: ["legacy/**"] });
    expect(igCustom.ignores("db/schema.rb")).toBe(true);
    // Real Ruby files in db/ are NOT excluded — only schema.rb itself
    expect(igBare.ignores("db/migrate/20260101_create_users.rb")).toBe(false);
    expect(igBare.ignores("db/seeds.rb")).toBe(false);
  });
  ...
  it("always excludes vendor/ directories regardless of options", () => {
    const igBare = buildCodegraphExclusionFilter({ customPatterns: [] });
    expect(igBare.ignores("vendor/assets/javascripts/d3.js")).toBe(true);
    expect(igBare.ignores("vendor/bundle/ruby/3.0/gems/foo-1.0/lib/foo.rb")).toBe(true);
    expect(igBare.ignores("app/vendor/lib/some.rb")).toBe(true);
    // Unrelated custom patterns don't disturb the invariant.
    const igCustom = buildCodegraphExclusionFilter({ customPatterns: ["legacy/**"] });
    expect(igCustom.ignores("vendor/assets/javascripts/d3.js")).toBe(true);
    // Real project files outside vendor/ are NOT excluded.
    expect(igBare.ignores("app/models/user.rb")).toBe(false);
  ...
  describe("per-language exclusion globs aggregated from the injected LanguageFactory", () => {
    it("aggregates a provider's codegraphExclusionGlobs when a factory is injected", () => {
      ...
      const ig = buildCodegraphExclusionFilter({ customPatterns: [] }, factory);
    ...
    it("layers language globs alongside test + custom patterns", () => {
      ...
      const ig = buildCodegraphExclusionFilter({ customPatterns: ["**/generated/**"] }, factory);
    ...
    it("excludes Rails non-app db/ paths when wired with the real LanguageFactory (ruby owns the globs)", () => {
      const ig = buildCodegraphExclusionFilter({ customPatterns: [] }, languageFactory());
      expect(ig.ignores("db/migrate/20260101_create_users.rb")).toBe(true);
      expect(ig.ignores("db/data/20260101_backfill.rb")).toBe(true);
