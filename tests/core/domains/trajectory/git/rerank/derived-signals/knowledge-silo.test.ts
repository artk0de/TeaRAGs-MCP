...
// chunkCC=3, fileCC=6 → coverage 0.5 × maturity 1 → alpha 0.5.
// blend = 0.5*chunk + 0.5*file. commitCount(file)=6 → dampening (6/5)^2 → 1.
...
describe("KnowledgeSiloSignal — piecewise-linear over blended contributor count", () => {
  ...
  it("mixed file=2 chunk=1 blend (a non-integer) lands in the 0.5–1.0 silo band (was 0)", () => {
    const raw = payload({ commitCount: 6, blameContributorCount: 2 }, { commitCount: 3, blameContributorCount: 1 });
  ...
  it("regression: integer 1 contributor → 1.0", () => {
    const raw = payload({ commitCount: 6, blameContributorCount: 1 }, { commitCount: 3, blameContributorCount: 1 });
    expect(signal.extract(raw, ctx)).toBeCloseTo(1.0, 6);
  });

  it("regression: integer 3 contributors → 0", () => {
    const raw = payload({ commitCount: 6, blameContributorCount: 3 }, { commitCount: 3, blameContributorCount: 3 });
    expect(signal.extract(raw, ctx)).toBe(0);
  });

  it("file-only (alpha=0) integer 1 → 1.0", () => {
    const raw = payload({ commitCount: 6, blameContributorCount: 1 });
    expect(signal.extract(raw, ctx)).toBeCloseTo(1.0, 6);
  });

  it("interpolation contract: blended 2.5 → 0.25", () => {
    const raw = payload({ commitCount: 6, blameContributorCount: 3 }, { commitCount: 3, blameContributorCount: 2 });
  ...
  it("returns 0 when no contributor data", () => {
    expect(signal.extract(payload({ commitCount: 6 }), ctx)).toBe(0);
  });
});
