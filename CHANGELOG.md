...
* New named filter presets — pass, for example, `{ presets: "godMethods" }` to a search/rank tool instead of writing raw filter JSON. Presets cover common code-quality investigations: security-sensitive paths and core production logic, high-churn/high-risk files from git history (freshLegacyEdits, fragileSilo, panicZone, godMethods), call-graph structure (hubs, dead-code candidates, unstable core), and composite investigations (battle-tested code, abandoned hotspots). Available preset names are discoverable directly from the tool's schema.
* Two new ranking presets help find structurally risky code directly: decomposition surfaces outsized files and classes that are candidates for splitting up, and godModule surfaces files with too many symbols, weighted by how much of the codebase depends on them when call-graph data is available.
* Size-based rankings (e.g. finding oversized files or classes) now also honor fixed, per-language industry-standard size limits (such as ESLint's 300-line guideline or RuboCop's 100-line class limit) as a floor under the project-relative percentile, so a genuinely bloated file is flagged as large even on a codebase where everything is oversized.

### 🧠 Code intelligence

* Ruby call-graph resolution now recognizes variables named after their class — Rails' dominant naming convention (e.g. `user`, `actor`, `client`) — and resolves method calls on them accordingly, instead of leaving them unresolved.
* Ruby call-graph resolution now understands methods documented as returning either a type or nil (e.g. ``@return` [Firm, nil]`), instead of dropping that information and leaving the call unresolved.
* Call-graph data now self-repairs on every incremental reindex: files whose graph rows have drifted out of sync with the code are automatically detected and re-extracted, instead of accumulating stale or missing call-graph data indefinitely. This also fixes a bug that made every incremental reindex silently re-parse the entire repository instead of just the drifted files.

### 🗣 Language support

* Ruby call-graph resolution now understands common Rails-ecosystem gem conventions — Devise's scoped current-user-style accessors, CanCanCan authorization checks, Draper decorators, ActiveModelSerializers pass-through attributes, Kaminari and will_paginate pagination chains, Ransack search chains, and dry-initializer declared readers — instead of treating the values they return as untyped.

### 🩹 Fixes

* Fixed two bugs in call-graph enrichment tracking: files excluded by custom exclusion patterns no longer get stuck reporting a permanent "degraded" status on every incremental reindex, and files skipped during a run are now marked as skipped immediately instead of one run later.
* Call-graph short-name lookups (used by tools like find_callers) now correctly resolve compactly-namespaced class declarations (e.g. `Module::Class`), instead of returning no results for them.
* Ruby call-graph resolution now follows Ruby's real method-resolution order across mixins and multi-level inheritance — `super` calls, inherited return-type facts, and ambiguous method lookups now resolve to the correct ancestor instead of sometimes picking a farther-away superclass over a nearer mixin.
* Ranking overlay labels (e.g. distinguishing an ordinary file from a "hub" or an oversized "god module") now render correctly for several structural signals that previously showed as unlabeled raw numbers.
* Call-graph queries (like get_callers) no longer hang indefinitely when the background call-graph process has gone idle — they now fail fast and reconnect automatically on the next call.
* Cloning a project's call-graph index (e.g. for a git worktree) no longer silently loses recently-computed data such as fan-in and centrality scores.
* Call-graph tools (get_callers, get_callees, find_cycles) now report an error when the underlying graph can't be read, instead of silently returning an empty result that looks identical to "this symbol truly has no callers."
* Indexing a git worktree no longer silently falls back to an unreachable default embedding endpoint — it now inherits the same embedding configuration as its main checkout.
* Fixed a case where the recovery phase of an incremental reindex could fail to enrich files with call-graph data because the background call-graph process wasn't being kept alive for that phase.
* Large call-graph query responses now decode dramatically faster (up to roughly 150x on multi-megabyte results), and non-ASCII identifiers in those responses no longer get corrupted.
* Disk space used by orphaned call-graph database files left behind by renamed or aliased projects is now reclaimed automatically.
* Incrementally reindexing a renamed (aliased) project no longer risks writing call-graph data into the wrong underlying collection.
* Indexing a git worktree now inherits its main checkout's index settings (embedding backend, chunk size, call-graph config) instead of borrowing whatever project happened to be indexed most recently.
* Indexing a fresh git worktree of an already-indexed repository now reuses the existing git-blame cache instead of rebuilding it from scratch.

### 🔧 Environment Variables

* `CODEGRAPH_EXCLUDE_TESTS` · Removed — codegraph indexing now always excludes test files (this was already the default behavior); setting this variable no longer has any effect. · default: `true (fixed, no longer configurable)` (changed)
* `CODEGRAPH_BULK_FILES` · Batch size for call-graph pass-2 file writes, for tuning throughput on large repositories. · default: `256` (new)
* `CODEGRAPH_CHECKPOINT_EVERY` · How often (in files) the call-graph pass-2 writer checkpoints to disk. · default: `500` (new)
