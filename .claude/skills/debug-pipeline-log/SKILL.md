...
5. **GitEnrich phases** — PREFETCH_START, PREFETCH_COMPLETE, STREAMING_APPLY,
   FLUSH_APPLY, CHUNK_CHURN_START/COMPLETE, ALL_COMPLETE
...
- **Git log prefetch duration**: PREFETCH_COMPLETE.durationMs (CLI
  `git log --numstat HEAD`)
...
- **Backpressure events**: count of BACKPRESSURE_ON / BACKPRESSURE_OFF, total
...
- **Path match rate**: matchedFiles / (matchedFiles + missedFiles) — should
  be >90%
- **Streaming vs flush applies**: streamingApplies (during embedding) vs
  flushApplies (after)
...
If `activeWorkers < EMBEDDING_CONCURRENCY` for extended periods (no
backpressure):
...
If `streamingApplies == 0` and `flushApplies > 0`:

- **Diagnosis**: Enrichment completed only after embedding finished (no overlap)
  — ran effectively sequentially
- **Impact**: Prefetch duration adds directly to total wall time instead of
  being hidden behind embedding
- **Fix**: Verify `CODE_ENABLE_GIT_METADATA=true` and that enrichment starts
  before embedding begins
...
- **Fix**: Increase `GIT_CHUNK_TIMEOUT_MS` (default 120s), reduce
  `GIT_CHUNK_MAX_AGE_MONTHS`, or set `GIT_CHUNK_ENABLED=false` for very large
  repos
...
| Streaming applies | X during / Y after embedding | OK/WARN |
