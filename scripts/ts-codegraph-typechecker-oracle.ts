...
import {
  ...
  diffResolution,
  isNonSourceTarget,
  isOutsideProjectSource,
  ORACLE_UNLOCATED_SHAPES,
  tallyBy,
  type OracleAnswer,
  type OracleOutcome,
  type OracleRow,
  ...
  type OracleTargetOrigin,
  type OracleUnlocatedShape,
  type OracleVerdict,
} from "./lib/codegraph-oracle-core.js";

// ---------------------------------------------------------------------------
// Pure core — the diff, the tally, the ranking. Unit-tested in
// tests/scripts/ts-codegraph-typechecker-oracle.test.ts.
//
// The row / verdict vocabulary, `diffResolution` and `tallyBy` live in
// `lib/codegraph-oracle-core.ts`, which the Python oracle shares; they are
// re-exported here so this script stays their address for its importers.
// ---------------------------------------------------------------------------
