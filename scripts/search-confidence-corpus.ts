...
function report(corpus: Corpus, fit: Fit): boolean {
  ...
  console.log(`                  ${NONSENSE_SNIPPETS.length} + ${LEGIT_SNIPPETS.length} snippets for find_similar\n`);
  ...
  console.log(`  nonsense  max ${round2(Math.max(...shippedNonsense))}  p90 ${round2(quantile(shippedNonsense, 0.9))}`);
