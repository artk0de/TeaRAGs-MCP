...
const blogPosts = existsSync("blog-posts.json") ? JSON.parse(readFileSync("blog-posts.json", "utf8")) : [];

const section = renderChangelogSection(data, { blogPosts });
writeFileSync("release-notes.md", renderReleaseNotes(data, contributors, { blogPosts }));

const changelog = readFileSync("CHANGELOG.md", "utf8");
writeFileSync("CHANGELOG.md", spliceVersionSection(changelog, data.version, section));

console.error(`built artifacts for v${data.version} (${blogPosts.length} blog post(s) in range)`);
