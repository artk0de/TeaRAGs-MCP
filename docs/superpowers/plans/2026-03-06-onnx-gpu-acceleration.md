...
**Issue:** tea-5eff **Priority:** P1 **Context:** ONNX WASM/CPU = 3.7 chunks/sec
(501s for 134 texts). Ollama GPU = full index in 12s.

## Problem

`@huggingface/transformers` v3 in Node.js defaults to CPU. GPU acceleration
exists but only for specific platforms. No auto-detection.
...
`"auto"` already exists in transformers.js — it picks the best EP for the
platform. This alone enables CUDA on Linux + DirectML on Windows without any
custom logic.
...
3. Windows x64: `"auto"` → `"dml"` (DirectML for any GPU: NVIDIA, AMD, Intel
   Arc)
4. Benchmark each
