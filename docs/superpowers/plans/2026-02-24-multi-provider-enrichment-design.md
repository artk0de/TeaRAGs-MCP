...
**Date:** 2026-02-24 **Status:** Approved **Related:** tea-rags-mcp-q9f (config
refactor — separate epic)
...
This blocks adding new enrichment providers (code complexity, dependency graph,
etc.).
...
Each public method (`prefetch`, `onChunksStored`, `startChunkEnrichment`,
`awaitCompletion`) iterates states and runs providers in parallel via
`Promise.all`.
...
Delete hardcoded `git.file.*` payload construction (lines 345-360). File-level
metadata is already written by EnrichmentApplier via `onBatchUpserted` callback.
...
**Unchanged:** BaseIndexingPipeline, EnrichmentApplier, EnrichmentProvider
interface, GitEnrichmentProvider, indexing.ts, reindexing.ts.
...
- New test: coordinator with two mock providers — parallel prefetch,
  onChunksStored, awaitCompletion
- Verify ChunkPipeline payload no longer contains `git.file.*`
- Existing enrichment-await integration tests should pass (single git provider
  in array)
