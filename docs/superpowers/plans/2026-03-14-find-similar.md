...
function createMockEmbeddings(): EmbeddingProvider {
  return {
    embedBatch: vi
      .fn()
      .mockResolvedValue([{ embedding: [0.1, 0.2, 0.3], dimensions: 3 }]),
    embed: vi.fn(),
...
describe("SimilarSearchStrategy", () => {
  ...
  it("converts fileExtensions to Qdrant filter with match.any", async () => {
    ...
    const callArgs = (qdrant.query as ReturnType<typeof vi.fn>).mock
      .calls[0][1];
...
export class SimilarSearchStrategy extends BaseExploreStrategy {
  ...
  protected async executeExplore(
    ...
    const positiveCodeBlocks = (this.input.positiveCode ?? []).filter(
      (c) => c.trim().length > 0,
    );
    const negativeCodeBlocks = (this.input.negativeCode ?? []).filter(
      (c) => c.trim().length > 0,
    );
...
**REQUIRED: Promote local variables to instance fields.** The existing
constructor has `const signals = deps.payloadSignals ?? []` and
`const keys = deps.essentialKeys ?? []` as local variables (lines 93-94). These
must become instance fields so `findSimilar()` can access them.
...
Add to `createSearchSchemas()` function (after `RankChunksSchema`, before the
return statement):
...
const FindSimilarSchema = {
  ...
  fileExtensions: z
    ...
    .optional()
    .describe(
      "Filter by file extensions (e.g. ['.ts', '.js']). Shorthand for Qdrant fileExtension filter.",
    ),
  ...
  metaOnly: coerceBoolean()
    .optional()
    .describe(
      "Return only metadata (path, lines, git info) without content. Default: false.",
    ),
};
```
...
server.registerTool(
  ...
  async ({
    ...
    try {
      ...
      return appendDriftWarning(
        formatMcpResponse(response.results),
        response.driftWarning,
      );
    } catch (error) {
...
> Note: This task depends on Task 5 (presets).
> `buildRerankSchema("find_similar")` requires at least one preset to list
> `"find_similar"` in its `tools[]`.
