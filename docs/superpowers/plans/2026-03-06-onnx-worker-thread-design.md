...
**Goal:** Move ONNX embedding inference to a dedicated `worker_threads` thread
so the main thread event loop stays responsive during CPU-bound inference.

**Problem:** ONNX is the only in-process embedding provider. When it runs
inference on CPU, it blocks the Node.js event loop for seconds to minutes. This
causes:
...
All other providers (Ollama, OpenAI, Cohere, Voyage) use HTTP APIs and don't
...
- `OnnxEmbeddings` keeps the same public API (`EmbeddingProvider` interface) —
...
3. Model loaded in worker on first `embed` message (lazy, ~5-10s, may download
   ~70MB)
...
- **Create:** `src/core/adapters/embeddings/onnx/worker.ts` — worker thread
  entry point
- **Modify:** `src/core/adapters/embeddings/onnx.ts` — proxy that delegates to
  worker
- **Keep:** `src/core/adapters/embeddings/onnx/coreml.ts` — unchanged (will move
  to worker later)
...
- **Con:** Structured clone overhead for embeddings (~1.5MB per batch of
  256×768) — negligible
- **Con:** Worker startup adds ~50-100ms on first call — acceptable for lazy
  init
