...
**Scope:** symbol-mass payload signals, `godClass` preset, composite
`decomposition` preset, `risk-assessment` skill rework
...
| B   | `symbolCount` derived signal + `godClass` preset              | `static/rerank/`           | uses A  |
| C   | Composite `decomposition` preset (`chunkFanOut`)              | `composite/presets/`       | no      |
...
Constraint: `fileSymbolCount` percentiles MUST be computed over distinct files
(dedupe by `relativePath`), not over chunks — otherwise many-chunk files
dominate their own distribution. Mechanism follows the existing `git.file.*`
handling; exact wiring is a plan-level detail.
...
## B. `godClass` preset (static, file-level)
...
```ts
// src/core/domains/trajectory/static/rerank/presets/god-class.ts
name = "godClass";
...
Static trajectory, not composite: the preset needs neither git nor codegraph, so
it works on any collection once reindexed. "Top-10 god classes of the project"
becomes one call: `rank_chunks rerank=godClass limit=10`.
...
`fanIn` / `pageRank` are deliberately NOT in the weights: they measure
call-graph connectivity, not interface mass. They remain inputs to the skill's
fix-cost classifier (Part D), read from other presets' overlays.
...
1b STRUCTURAL     decomposition + godClass  ─┘ (+2 calls)
...
- `rerank: "godClass"` — file axis. Overlay carries `fileSymbolCount` /
  `memberCount` / `classLines`.
...
`fileSymbolCount` straight from the `godClass` overlay — zero extra calls. Apply
the attribution rule from Part B.
...
- **godClass** — registered in static presets; resolves for all four tools;
...
3. Self-test: `npm run build && npm link` (single-worktree rule), reconnect MCP,
   ...
   `fileSymbolCount`, `rank_chunks rerank=godClass` returns ranked files, full
