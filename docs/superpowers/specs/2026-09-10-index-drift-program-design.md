...
6. **Env drift compares the stamp against the env the next index run would
   ...
   keeps the effective-env compare. Amended by the V1 live validation (`lg361`):
   `run-prime.ts` re-applies `CODEGRAPH_ENABLED` from the registry identity
   field before `parseAppConfig`, so the composition inside a `tea-rags prime`
   run always carries the flag and that one finding cannot appear there at all.
   It is a claim about the MCP SERVER process and is verified there, through
   `get_index_status` or a search `driftWarning`; `TRAJECTORY_GIT_ENABLED` is
   not re-applied and was confirmed in prime. Amended by `o0qsw` (2026-09-17):
   "outer env" depends on where the ambient env came from (`AmbientEnvRole`). A
   CLI invocation's shell env still overrides every group. A long-lived MCP
   server's spawn env is every served project's DEFAULT: for a project's own
   registry entry it outranks the stamp only in `runtime` groups, and yields
   every `chunk-set` / `enrichment:*` group the stamp pins to a different value
   (`outerEnvForRegistryStamp`). The server entry points declare the role;
   `ProjectIngestFactory` and this monitor resolve under the same one, and the
   auto-update run the server detaches inherits the already-narrowed env
   (`outerEnvForRegistryEntry`), since it replays as an invocation. The reason
   is the phantom this decision already named, which replay alone did not
   prevent for keys the server env sets: a server spawned with the tea-rags-mcp
   session's `CODE_CHUNK_SIZE=2000` reported `INGEST_CHUNK_SIZE: 4500 → 2000` on
   taxdome for every search, its `--force` remedy run from a shell re-stamped
   4500 and could never clear it, and an MCP `index_codebase` on taxdome would
   really have chunked at 2000 over a 4500 index.
...
| C    | exporting `CODEGRAPH_AMBIGUOUS_RESOLVE_MODE` in the shell of a `tea-rags prime` run to a value that differs from the stamp (an invocation override, decision 6) produces an env finding on the self-index with no reindex, and removing the override clears it — the same value in the MCP server env produces none (a server's spawn env is a default, `o0qsw`); a commit without reindex produces a commit finding; a `*` bump produces a finding with no `--languages`; the canary rejects a second model under the same tag on a fixture. The `CODEGRAPH_ENABLED` flip is evidence only from an MCP server started without the flag — prime re-applies it (decision 6) |
