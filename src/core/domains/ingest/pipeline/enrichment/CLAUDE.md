...
- **Every payload write is scoped by `op.key` to `` `${providerKey}.file` `` or
  ...
  `EnrichmentApplier` (`#applySkipStamps`, `#matchedFileEntries`,
  `#unmatchedFileEntries`, `#applyFinalizeFile`, `#applyChunkSignals`),
  `EnrichmentBackfiller` (`#runFor`, `#backfillChunkSignals`), `recovery.ts`,
  and any schema migration
...
- **Codegraph is pinned to one worker thread by `routingKey = collectionName`**
  ...
  `src/bootstrap/factory.ts`) — affinity measured ~4× slower for it;
  `"stateless"` exists in the type, unused. Release is explicit (no LRU, no idle
  timeout) and `releaseEntry` (`infra/worker.ts`) deletes the cache entry BEFORE
  awaiting `onRelease`, so a throw still bounds memory. The pool disables the
  liveness timeout (`WorkerPoolEnrichmentExecutor#constructor`). Why: calling
  git "stateless dispatch" is wrong both ways — git never reaches the pool, and
  a provider marked `stateless` round-robins and loses affinity state mid-run.
  An affinity worker crash still loses that collection's accumulated run state —
  only the recovery scan heals THAT — but the pool slot itself is respawned, so
  the next dispatch is not posted to a dead handle.
...
- **`enrichedAt` is the run's `startedAt`, not a write timestamp** — threaded
  ...
  longer means "never reached", and a point carrying both stamps is a
...
- **Streamable per stored batch: git file signals, git chunk signals, codegraph
  ...
  (`contracts/types/provider.ts`); where the deferred chunk pass sits in
...
- **`classify()` (`core/infra/file-classification/`) is the FACT; `shouldEnrich`
  ...
  (`GitEnrichmentProvider#shouldEnrich` — ownership is legitimate); codegraph
