...
export class PythonLocalBindingSymbolResolutionStrategy implements SymbolResolutionStrategy {
  ...
  private resolveByLocalType(typeName: string, member: string, ctx: CallContext): SymbolResolutionTarget | null {
    ...
    // half `chainType` shares — one implementation in `shared.ts`. What stays
    // HERE is step 4, which is this pass's own measured policy.
    const onType = resolvePythonMemberOnType(typeName, member, ctx, this.cfg.mode, this.mapper);
