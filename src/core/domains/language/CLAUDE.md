...
- **Import → file is one seam per language.** `ImportFileMapper`
  ...
  (and the cone locator through it), the external vocabulary. The mapper answers
  from `GlobalSymbolTable.hasFile` / `hasFilesUnder`, never from disk, and a PEP
  420 namespace package answers `unknown` because a directory is not a legal
  ...
  disk); migrating it is a follow-up. Chain index 5, `importedName`, sits
  between `localBinding` and `importMatch` — it reads the binding the walker
  recorded, `importMatch` guesses from the module's trailing segment, and the
  knower must run first.
