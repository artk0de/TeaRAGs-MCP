/**
 * Test-Spec Scope Chunker for JavaScript — Groups Vitest/Jest/Mocha-style
 * test specs by scope hierarchy. Mirror of ../typescript/chunking/
 * test-scope-chunker.ts against the tree-sitter-javascript grammar (same
 * node vocabulary: call_expression + arrow_function/function_expression
 * callbacks + statement_block bodies), adapted per the canonical structure
 * in `.claude/rules/test-spec-chunking.md`. bd tea-rags-mcp-1etj8.
 *
 ...
 * beforeAll, etc.) from the ancestor scopes for self-contained context.
 *
 * Chunks get a 2-level symbolId: "TopLevelDescribe.leafScopeName".
 *
 * ── Language-list pointer (MANDATORY) ────────────────────────────────
 * This hook emits `chunkType: "test"` / `"test_setup"` for JavaScript.
...
export function buildScopeTree(containerNode: AstNode, code: string): TestScope {
  ...
  // statement_block in tree-sitter-javascript includes the surrounding `{`
...
export const jsTestScopeChunkerHook: ChunkingHook = {
  name: "js-test-scope-chunker",
