 ...
 * base snapshot this process loaded. A field present in base and dropped in
 ...
 * cache.
 */
function mergeChangedFields(disk: CollectionEntry, mem: CollectionEntry, base: CollectionEntry): CollectionEntry {
  ...
  for (const key of new Set([...Object.keys(memFields), ...Object.keys(baseFields)])) {
    if (isDeepStrictEqual(memFields[key], baseFields[key])) continue;
 ...
 *
 * Optional `base` is the snapshot of the entries as they were when this
 * process loaded the file. With it, an overlapping entry is merged field by
 * field (`mergeChangedFields`) — only what changed since load wins — and an
 * entry unchanged since load that another process deleted is NOT resurrected.
 * Without a base entry the process either created the entry or wrote it whole,
 * so in-memory wins outright through mergeRegistryEntries.
 *
...
export function mergeRegistryDelta(
  ...
  base?: ReadonlyMap<string, CollectionEntry>,
): RegistryFileV1 {
  ...
  for (const [k, mem] of delta.entries()) {
    const baseEntry = base?.get(k);
    const onDisk = out[k];
    if (baseEntry === undefined) {
      out[k] = onDisk ? mergeRegistryEntries(onDisk, mem) : mem;
    } else if (onDisk !== undefined) {
      out[k] = mergeChangedFields(onDisk, mem, baseEntry);
    } else if (!isDeepStrictEqual(mem, baseEntry)) {
      out[k] = mem;
    }
  }
  if (tombstones) {
 ...
 *
 * Optional `base` is the caller's snapshot of the entries at load time; it
 * turns the per-entry merge into a three-way one (see mergeRegistryDelta).
 * Returns the file that was written, so the caller can refresh its base.
 *
...
export function flushWithCAS(
  ...
  for (let attempt = 0; attempt < CAS_MAX_ATTEMPTS; attempt++) {
    ...
    const merged = mergeRegistryDelta(disk, delta, tombstones, base);
