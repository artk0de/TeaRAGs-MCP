 ...
 *
 * These functions extract values from the search result payload,
...
/**
 * Read a field from file-level git metadata, returning undefined if absent.
 */
...
/**
 * Read a numeric field from file-level git metadata, returning 0 if absent.
 */
export function fileNum(payload: Record<string, unknown>, field: string): number {
  const v = fileField(payload, field);
  return typeof v === "number" ? v : 0;
}

/**
 * Read a numeric field from chunk-level git metadata, returning 0 if absent.
 */
...
/**
 * Check if a specific chunk field is defined.
 */
export function hasChunkField(payload: Record<string, unknown>, field: string): boolean {
  const chunk = resolveChunk(payload);
...
export function computeAlpha(payload: Record<string, unknown>): number {
  ...
  // Prefer chunk commit count, fall back to file
